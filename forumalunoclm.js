window.Router.register('forumalunoclm', async () => {
        ...
        <div id="modal-edit-forum" style="position:fixed; top:0; left:0; width:100%; height:100%; background:rgba(15, 23, 42, 0.75); display:flex; justify-content:center; align-items:center; z-index:9999; backdrop-filter: blur(6px); animation: fadeInModal 0.2s ease;">
            <div style="background:white; border-radius:25px; max-width:500px; width:90%; overflow:hidden; box-shadow: 0 20px 50px rgba(0,0,0,0.2);">
                ...
                <div style="padding:25px; display:flex; flex-direction:column; gap:15px;">
                    <textarea id="novo-texto-forum" autocomplete="off" style="padding:15px; border-radius:15px; border:1px solid #e2e8f0; min-height:150px; font-family:inherit; width:100%; box-sizing:border-box; resize:none; font-size:0.95rem; outline:none;">${textoAntigo}</textarea>
        ...
        if (novoTexto && novoTexto !== textoAntigo) {
            try {
                await updateDoc(doc(db, "forum_comentarios", id), { 
                  texto: novoTexto, editado: true, dataEdicao: serverTimestamp() 
                });
  ...
  const iniciarEscutaForumAluno = () => {
    ...
    const q = query(collection(db, "forum_topicos"), orderBy("dataCriacao", "desc"));
  ...
  window.renderizarListaAluno = () => {
    ...
    const processarLayout = async () => {
        const snap = await getDocs(query(collection(db, "forum_comentarios"), where("autorId", "==", auth.currentUser.uid)));
        const idsComentados = snap.docs.map(d => d.data().topicoId);

        container.innerHTML = itensExibidos.map((t) => {
            const jaComentei = idsComentados.includes(t.id);
   ...
   let htmlPaginacao = '';
    if (totalPaginas > 1) {
        htmlPaginacao += `
            <button class="btn-paginacao" ${paginaAtual === 1 ? 'disabled style="opacity:0.5; cursor:not-allowed"' : ''} onclick="window.mudarPaginaAluno(${paginaAtual - 1})">
        ...
        for (let i = 1; i <= totalPaginas; i++) {
            if (i === 1 || i === totalPaginas || (i >= paginaAtual - 1 && i <= paginaAtual + 1)) {
                ...
                htmlPaginacao += `<span style="color:#003058; font-weight:800;">...</span>`;
            }
        }

        htmlPaginacao += `
            <button class="btn-paginacao" ${paginaAtual === totalPaginas ? 'disabled style="opacity:0.5; cursor:not-allowed"' : ''} onclick="window.mudarPaginaAluno(${paginaAtual + 1})">
  ...
  window.abrirChatAluno = (id) => {
    ...
    document.getElementById('foco-cabecalho-aluno').innerHTML = `
      ...
      <div class="pergunta-prof-discreta">
        ...
        <div class="descricao-foco">${topico.descricao}</div>
        ...
        <div id="input-direto-main-aluno" class="reply-input-container">
            <input type="text" id="campo-main-aluno" placeholder="Escreva seu comentário..." autocomplete="off" onkeyup="if(event.key==='Enter') window.enviarMsgAluno('${id}', 'main', '${topico.autor}', '${topico.titulo.replace(/'/g, "\\'").replace(/"/g, '&quot;')}')">
            <button onclick="window.enviarMsgAluno('${id}', 'main', '${topico.autor}', '${topico.titulo.replace(/'/g, "\\'").replace(/"/g, '&quot;')}')"><i class="fa-solid fa-paper-plane"></i></button>
        </div>
      </div>
    `;
          ...
          <div class="msg-wrapper-clm ${souEu ? 'minha-msg' : 'aluno'}" id="msg-${cid}">
            <div class="bubble-clm ${eProfessor ? 'bubble-prof' : ''}">
                ...
                <div id="input-direto-${cid}" class="reply-input-container">
                  <input type="text" id="campo-${cid}" placeholder="Responder..." autocomplete="off" onkeyup="if(event.key==='Enter') window.enviarMsgAluno('${id}', '${cid}', '${c.autor}', '${c.texto.replace(/'/g, "\\'").replace(/"/g, '&quot;')}')">
                  <button onclick="window.enviarMsgAluno('${id}', '${cid}', '${c.autor}', '${c.texto.replace(/'/g, "\\'").replace(/"/g, '&quot;')}')"><i class="fa-solid fa-paper-plane"></i></button>
                </div>
            </div>
          </div>
        `;
  ...
  return `
    <style>
      ...
      .btn-paginacao { border: 1px solid #e2e8f0; background: white; color: #003058; padding: 6px 12px; border-radius: 6px; cursor: pointer; font-weight: 700; font-size: 12px; }
