    ...
    const carregarDadosHomeAluno = async () => {
        try {
            ...
            const monitorarAtividadesSync = (tipo, elId, cardId, globalVar) => {
                // Ajustado para buscar sempre em respostas_alunos, conforme o padrão do professor
                const qResp = query(collection(db, "respostas_alunos"), where("alunoId", "==", user.uid), where("tipo", "==", tipo));

                onSnapshot(qResp, (snapResp) => {
                    const respondidasIds = snapResp.docs.map(d => {
                        const data = d.data();
                        // Mapeia todos os possíveis campos de ID de referência
                        return data.atividadeId || data.idAtividade || data.propostaId || data.id;
                    });
                    ...
                    onSnapshot(qAtv, (snapAtv) => {
                        let pendentes = 0;
                        snapAtv.docs.forEach(docAtv => {
                            const atv = docAtv.data();
                            const idAtv = docAtv.id;
                            
                            const tRaiz = (atv.turma || "").toString().trim();
                            const sRaiz = (atv.semestre || "").toString().trim();
                            const tArray = Array.isArray(atv.turmasSelecionadas) ? atv.turmasSelecionadas : [];

                            const pertenceAoAluno = 
                                (turmaAluno !== "" && (tRaiz === turmaAluno || tArray.includes(turmaAluno))) ||
                                (semestreAluno !== "" && (tRaiz === semestreAluno || sRaiz === semestreAluno));

                            if (pertenceAoAluno && !respondidasIds.includes(idAtv)) {
                                pendentes++;
                            }
                        });
                        ...
                        if (card) {
                            if (pendentes > 0 && pendentes > ultimoVisto) {
