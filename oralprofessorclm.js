    ...
    window.carregarAtividadesEnviadasOral = async () => {
        ...
        try {
            ...
            const htmlCards = itemsPagina.map(data => {
                const prazoFormatado = new Date(data.prazo.includes('T') ? data.prazo : data.prazo + "T23:59:59").toLocaleDateString('pt-BR');
                return `
                <div class="card-premium-list" style="border-left:4px solid ${azulCLM};">
                    ...
                    <div style="display:flex; align-items:center; gap:10px; margin-left:20px; flex-shrink:0;">
                        <span style="font-size:11px; font-weight:700; color:#64748b;">${prazoFormatado}</span>
