 ...
 window.criarTopicoForum = async () => {
   ...
   const btnPublicar = document.getElementById('btn-publicar-forum');
    ...
    try {
      ...
      const nomeArquivo = `${auth.currentUser.uid}_${Date.now()}.html`;
      const caminho = `forum_descricois/${nomeArquivo}`;
...
    try {
    ...
    confirmarExclusaoModal(async () => {
        ...
        if (topico?.descricaoPath) {
          try {
            ...
            const storage = getStorage();
            // A regra do storage espera apenas o nome do arquivo após a pasta
            const storageRef = ref(storage, topico.descricaoPath);
            await deleteObject(storageRef);
          } catch (e) { 
            // Se o arquivo já não existir, não deve travar a exclusão do documento
            console.warn("Arquivo do Storage não encontrado ou erro ao deletar:", e); 
          }
        }
  ...
  window.criarTopicoForum = async () => {
    ...
    try {
      ...
      let caminho = topicoOriginal.descricaoPath;
      // Garante que o caminho contenha o diretório e o UID no nome do arquivo
      if (!caminho || !caminho.includes(auth.currentUser.uid)) {
        caminho = `forum_descricois/${auth.currentUser.uid}_${Date.now()}.html`;
      }
