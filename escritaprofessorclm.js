   ...
   const { 
    ...
    try {
        const dados = {
            ...
            turma: senhaTurma, 
            imagemApoio: (contImg && contImg.style.display !== 'none') ? imgEl.src : "",
            dataEnvio: serverTimestamp(),
            professorId: userAtual.uid
        };

        if (idEditando) {
            await updateDoc(doc(db, "atividades_enviadas", idEditando), dados);
        } else {
...
window.removerFotoEscrita = () => {
    ...
    try {
        ...
        // 2. Busca as redações pendentes
        const qRed = query(collection(db, "redacoes"), where("status", "==", "pendente"));
        const snapRed = await getDocs(qRed);

        const grupos = {};
        snapRed.forEach(d => {
            const red = d.data();
            const codTurma = red.turma;
            
            // SÓ ADICIONA AO GRUPO SE A TURMA PERTENCER AO PROFESSOR
            if (senhasAutorizadas.includes(codTurma)) {
...
window.verAlunosDaTurma = async (codTurma, nomeTurma) => {
    ...
    try {
        const q = query(collection(db, "redacoes"), where("status", "==", "pendente"), where("turma", "==", codTurma));
        const snap = await getDocs(q);
        recebidasCache = [];
        snap.forEach(d => {
            const data = d.data();
    ...
    modal.innerHTML = `
        <div style="background:#f8fafc; border-radius:16px; width:95%; max-width:850px; max-height:95vh; overflow-y:auto; padding:25px; position:relative; box-shadow:0 25px 50px -12px rgba(0,0,0,0.25);">
            ...
            <div id="area-comentario" style="display:none; margin-top:20px; padding:20px; background:#fff; border-radius:12px; border:1px solid #e2e8f0; animation: fadeIn 0.3s ease;">
                ...
                <textarea id="input-feedback-prof" style="width:100%; min-height:100px; border:1px solid #cbd5e1; border-radius:8px; padding:12px; font-family:inherit; margin-bottom:15px; outline:none; resize:vertical;" placeholder="Escreva aqui os pontos positivos e o que o aluno precisa melhorar..."></textarea>
                <button onclick="window.enviarFeedbackRedacao('${id}')" style="width:100%; background:#003058; color:white; border:none; padding:15px; border-radius:8px; cursor:pointer; font-weight:800; text-transform:uppercase; box-shadow: 0 4px 12px rgba(0,48,88,0.2);">FINALIZAR CORREÇÃO</button>
            </div>
        </div>`;
...
window.enviarFeedbackRedacao = async (redacaoId) => {
    ...
    try {
        const redRef = doc(db, "redacoes", redacaoId);
        ...
        // Atualiza a lista de recebidas para sumir o card de lá
        await window.carregarTodasRedacoesRecebidas(); 
        // Opcional: Se quiser que ele mude de aba automaticamente para ver o resultado:
        // window.switchMainTabEscrita('corrigidas');
        
    } catch (e) {
        console.error("Erro ao enviar feedback:", e);
        window.mostrarAvisoErro("Não foi possível enviar o feedback. Tente novamente.");
    }
};
...
window.carregarRedacoesCorrigidas = async () => {
    ...
    try {
        ...
        // Filtra pelo status 'corrigida' E pelo ID do professor logado
        const q = query(
        ...
        container.innerHTML = '<p style="color:red; padding:20px;">Erro ao carregar corrigidas. Verifique o console para criar o índice necessário.</p>'; 
    }
};
    ...
    <button id="tab-btn-corrigidas" class="main-tab-btn" onclick="window.switchMainTabEscrita('corrigidas')">CORRIGIDAS</button>
        ...
        <div id="pane-criar" class="tab-pane-escrita" style="display:block;">
            <div class="card-premium" style="padding: 25px;">
                ...
                <textarea class="input-premium" style="min-height: 120px;" placeholder="Digite aqui o tema..."></textarea>
