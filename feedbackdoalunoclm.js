window.Router.register('feedbackdoalunoclm', async () => {
    const db = window.db;
    const { collection, query, where, getDocs } = window.fsMethods;

    let todosFeedbacks = [];
    let paginaAtual = 1;
    const itensPorPagina = 5;
    const azulPadrao = "#003058"; // Cor padrão do sistema
    ...
    window.abrirModalFeedbackCompleto = (id) => {
        ...
        const isEscrita = item.origem === 'redacoes';
        ...
        if (isEscrita) {
            ...
            corpoModal = `
                <div style="background:white; padding:20px; border-radius:18px; border:1px solid #e2e8f0; box-shadow: 0 4px 6px rgba(0,0,0,0.02);">
                    <h4 style="margin:0 0 12px 0; font-size: 0.7rem; color:${azulPadrao}; text-transform:uppercase; font-weight:800; letter-spacing:0.5px;">Comentário do Professor:</h4>
                    <div style="font-size: 0.95rem; color:#334155; line-height:1.7;">${conteudo.replace(/\n/g, '<br>')}</div>
                </div>`;
        } else {
    ...
    const carregarTudo = async () => {
        ...
        try {
            // Busca Escrita
            const qEscrita = query(collection(db, "redacoes"), where("alunoId", "==", uid));
            const snapEscrita = await getDocs(qEscrita);
            const dadosEscrita = snapEscrita.docs
                .map(d => ({ 
                    id: d.id, ...d.data(), 
                    origem: 'redacoes', 
                    dataOrdenacao: d.data().timestamp?.seconds || 0,
                    dataExibicao: d.data().timestamp?.toDate ? d.data().timestamp.toDate().toLocaleDateString('pt-BR') : '---'
                }))
                .filter(d => d.status === 'corrigida' || d.feedbackProfessor);

            // Busca Oral
            const qOral = query(collection(db, "respostas_alunos"), where("alunoId", "==", uid), where("tipo", "==", "oral"));
            const snapOral = await getDocs(qOral);
            const dadosOral = snapOral.docs
                .map(d => ({ 
                    id: d.id, ...d.data(), 
                    origem: 'respostas_alunos', 
                    dataOrdenacao: d.data().dataEnvio?.seconds || 0,
                    dataExibicao: d.data().dataEnvio ? new Date(d.data().dataEnvio.seconds * 1000).toLocaleDateString('pt-BR') : '---'
                }))
                .filter(d => d.avaliado === true);

            todosFeedbacks = [...dadosEscrita, ...dadosOral].sort((a, b) => b.dataOrdenacao - a.dataOrdenacao);
            renderizarListaFeedbacks();
