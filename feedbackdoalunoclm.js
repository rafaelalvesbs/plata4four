    ...
    const carregarTudo = () => {
        ...
        onSnapshot(qResp, (snap) => {
            cacheRespostas = snap.docs.map(d => {
                ...
                const dataReferencia = data.dataEnvio || data.timestamp;
                const dataFormatada = dataReferencia?.toDate ? dataReferencia.toDate().toLocaleDateString('pt-BR') : '---';

                return { 
                    id: d.id, 
                    ...data, 
                    dataOrdenacao: dataReferencia?.seconds || 0, 
                    dataExibicao: data.dataEntrega || dataFormatada 
                };
            }).filter(i => i !== null);
            atualizarGeral();
        });
