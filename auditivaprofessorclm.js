window.Router.register('auditivaprofessorclm', async () => {
    ...
    window.handleAudioQuestao = (input, n) => {
        if (input.files && input.files[0]) {
            const file = input.files[0];
            if (file.size > 1 * 1024 * 1024) { 
                window.showAlertAud("Erro", "Áudio muito grande. Máximo 1MB por questão.", "error");
                return;
            }
    ...
    window.enviarAtividadeAuditiva = async (isAutoSave = false) => {
        ...
        try {
            let questoes = [];
            for(let i=1; i<=10; i++) {
                const enuncEl = document.getElementById(`enunc-aud-${i}`);
                const enunc = enuncEl ? enuncEl.value : "";
                
                if(enunc.trim()) {
                    const audioSrc = document.getElementById(`audio-src-aud-${i}`).src;
                    
                    if(!isAutoSave && (!audioSrc || !audioSrc.startsWith('data:audio'))) {
                        btnPub.disabled = false;
                        btnPub.innerHTML = originalText;
                        return window.showAlertAud("Erro na Questão " + i, "Você precisa adicionar um áudio para todas as questões que preencheu.", "error");
                    }

                    questoes.push({
                        enunciado: enunc,
                        opcoes: { 
                            A: document.getElementById(`opt-a-aud-${i}`).value, 
                            B: document.getElementById(`opt-b-aud-${i}`).value, 
                            C: document.getElementById(`opt-c-aud-${i}`).value 
                        },
                        correta: document.getElementById(`correct-aud-${i}`).value,
                        audioQuestao: audioSrc.startsWith('data:audio') ? audioSrc : null
                    });
                }
            }
...
<input type="text" id="opt-c-aud-${n}" class="input-premium" placeholder="C" oninput="window.triggerAutoSaveAud()" autocomplete="off">
        ...
        <div id="modal-alert-aud" class="modal-aud"><div class="modal-aud-content"><div id="alert-icon-aud" style="margin-bottom:10px;"></div><h3 id="alert-title-aud"></h3><p id="alert-msg-aud"></p><button class="btn-publish-aud" onclick="window.closeAlertAud()">OK</button></div></div>
