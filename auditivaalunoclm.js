window.Router.register('auditivaalunoclm', async () => {
    ...
    window.começarDeVezAud = () => {
       ...
       document.getElementById('atv-titulo-exec').innerText = atividadeAtual.titulo;
        // O áudio será carregado individualmente na função renderizarQuestaoAtualAud
    ...
    window.renderizarQuestaoAtualAud = () => {
        ...
        // Atualiza o áudio da questão atual
        const player = document.getElementById('player-aluno');
        const source = document.getElementById('player-aluno-src');
        ...
        if (q && q.audioQuestao) {
            ...
            // Se a questão não tiver áudio, esconde o player para não confundir
            if(headerAudio) headerAudio.style.display = 'none';
            if(player) player.pause();
        }

        // Renderizar Nav Pills (Bolinhas/Botões de navegação)
        ...
        // Renderizar Conteúdo (Visual Gramática)
        ...
        // Controle de visibilidade dos botões de navegação e finalizar
        const totalQuestoes = atividadeAtual.questoes.length;
        document.getElementById('btn-prev-aud').disabled = (indiceQuestaoAtiva === 0);
        
        // Esconde as setas se houver apenas 1 questão, caso contrário, exibe-as
