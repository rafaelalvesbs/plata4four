window.Router.register('oralalunoclm', async () => {
    ...
    window.enviarDadosFinaisOral = async () => {
        const btnEnvio = document.querySelector('#modal-confirm-envio-oral .btn-acao-enviar');
        if (btnEnvio) {
        ...
        try {
            ...
            const indices = Object.keys(respostasGravadas);
            
            for (let i = 0; i < indices.length; i++) {
                const idx = indices[i];
                const audioBase64 = await blobToBase64(respostasGravadas[idx]);
                questoesComAudio.push({
                    enunciado: atividadeSelecionada.questoes[idx].enunciado,
    ...
    <div class="gram-aluno-wrapper">
        ...
        <div id="modal-confirm-envio-oral" class="modal-aluno">
            <div class="modal-content-aluno">
                ...
                <div style="display:flex; gap:10px;">
                    <button onclick="document.getElementById('modal-confirm-envio-oral').style.display='none'" style="flex:1; background:#f1f5f9; color:#64748b; border:none; height:46px; border-radius:12px; font-weight:700; cursor:pointer;">REVISAR</button>
                    <button onclick="window.enviarDadosFinaisOral()" style="flex:1; background:${azulPadrao}; color:#fff; border:none; height:46px; border-radius:12px; font-weight:700; cursor:pointer;">ENVIAR</button>
                </div>
            </div>
        </div>
