window.Router.register('homemaster', async () => {
    ...
    const carregarDadosMaster = async () => {
        try {
            ...
            if (userDoc.exists()) {
                const nomes = (userDoc.data().nome || "Usuário Master").split(" ");
                const nomeExibicao = nomes.slice(0, 2).join(" ");
                document.getElementById('boas-vindas-master').innerText = `Olá, ${nomeExibicao}!`;
            }
            ...
            monitorar("usuarios", [where("perfil", "==", "Diretor")], "count-diretores", "card-diretores", "visto_master_diretores");
            monitorar("usuarios", [where("perfil", "==", "Coordenador")], "count-coordenadores", "card-coordenadores", "visto_master_coord");
            monitorar("usuarios", [where("perfil", "==", "Professor")], "count-professores", "card-professores", "visto_master_profs");
            monitorar("usuarios", [where("perfil", "==", "Aluno")], "count-alunos", "card-alunos", "visto_master_alunos");
