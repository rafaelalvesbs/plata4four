window.Router.register('calendario', async () => {
    ...
    const salvarEvento = async (data, titulo, idEdicao = null) => {
        ...
        try {
            ...
            // 1. Garantir que o documento existe no Firestore ANTES do Storage para validar a regra de segurança
            if (idEdicao && idEdicao !== 'null') {
                finalId = idEdicao;
                docRef = doc(window.db, "eventos", idEdicao);
            } else {
                const novoDoc = await addDoc(collection(window.db, "eventos"), {
            ...
            // 2. Agora o Storage permite o upload pois o documento com finalId já existe e pertence ao professor
            const storage = getStorage();
            const blob = new Blob([titulo], { type: 'text/plain' });
            const caminho = `calendario_eventos/${finalId}`;
            const storageRef = ref(storage, caminho);
            
            await uploadBytes(storageRef, blob);
            const urlTitulo = await getDownloadURL(storageRef);

            // 3. Atualiza o documento vinculando os dados do arquivo
