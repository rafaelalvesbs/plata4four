window.Router.register('gramaticaprofessorclm', async () => {
    ...
    window.enviarAtividadeGramatica = async (isAutoSave = false) => {
        ...
        if(!isAutoSave) {
            if(!semestreBtn || !titulo || !prazoInput) return window.showAlertGram("Atenção", "Preencha tudo!", "error");
            const dataHoje = new Date(); dataHoje.setHours(0,0,0,0);
            if (new Date(prazoInput + 'T00:00:00') < dataHoje) return window.showAlertGram("Data Inválida", "Prazo vencido!", "error");
        }
        try {
            let questoes = [];
            for(let i=1; i<=10; i++) {
            ...
            if(idEditandoGram) {
                ...
                if(!isAutoSave) { window.showAlertGram("Sucesso", "Atualizada!"); window.switchMainTabGram('enviadas'); }
            } else if(!isAutoSave) {
                const ref = await addDoc(collection(db, "atividades_enviadas"), dados);
                idEditandoGram = ref.id; window.showAlertGram("Sucesso", "Publicada!"); window.switchMainTabGram('enviadas');
            }
        } catch(e) { if(!isAutoSave) window.showAlertGram("Erro", "Falha ao salvar.", "error"); }
    };

    // --- EXCLUSÃO ---
    window.excluirAtividadeGram = (id) => {
        window.showConfirmGram("Excluir?", "Apagar permanentemente?", async () => {
            try { await deleteDoc(doc(db, "atividades_enviadas", id)); await window.carregarAtividadesEnviadasGram(); }
            catch (err) { window.showAlertGram("Erro", "Não foi possível excluir.", "error"); }
        });
    };
