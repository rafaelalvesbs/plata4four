...
          monitorarProximoEvento();
          ...
          const monitorarGramaticasRecebidas = () => {
            const q = query(collection(db, "respostas_alunos"), where("tipo", "==", "gramatica"));
            onSnapshot(q, (snap) => {
          ...
          const monitorarAuditivasRecebidas = () => {
            const q = query(collection(db, "respostas_alunos"), where("tipo", "==", "auditiva"));
            onSnapshot(q, (snap) => {
          ...
          const monitorarMensagensDiretas = () => {
            const qMsg = query(collection(db, "mensagens_diretas"), where("destinatarioId", "==", user.uid));
            onSnapshot(qMsg, (snap) => {
              const elContagem = document.getElementById('contagem-msg-recebidas');
              if (elContagem) {
                const mensagensFiltradas = snap.docs.filter(d => d.data().excluidoDestinatario !== true);
                const totalFinal = mensagensFiltradas.length;
                elContagem.innerText = totalFinal;
                window.msgCountHome = totalFinal;
                verificarAlertaPersistent('card-msg-recebidas', totalFinal);
              }
            });
          };
          monitorarMensagensDiretas();
  ...
  return `
   ...
   <div class="stats-grid">
      ...
      <div class="stat-card clickable" onclick="window.location.hash='#calendario'">
        ...
        <strong id="nome-prox-evento" style="font-size: 1rem; margin-top: 5px; display: block; white-space: nowrap; overflow: hidden; text-overflow: ellipsis;">...</strong>
    ...
    <div class="stats-grid">
      ...
      <div class="stat-card clickable" id="card-auditiva" onclick="window.acessarAtividade('card-auditiva', '#auditivaprofessorclm')">
        ...
        <strong id="contagem-auditiva">...</strong>
      </div>
    </div>
   ...
   <div class="stats-grid">
      <div class="stat-card clickable" id="card-msg-recebidas" onclick="window.acessarAtividade('card-msg-recebidas', '#mensagens')">
        ...
        <strong id="contagem-msg-recebidas">...</strong>
      </div>
