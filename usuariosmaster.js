window.Router.register('usuariosmaster', async () => {
  ...
  window.carregarListaUsuariosMaster = async () => {
    ...
    try {
      const q = query(collection(db, "usuarios"), where("perfil", "in", ["Diretor", "Coordenador", "Professor"]));
      ...
      usuariosFiltrados = Object.values(unicos).filter(u => !filtro || u.perfil === filtro);
      paginaAtual = 1;
      window.renderizarPaginaMaster();
